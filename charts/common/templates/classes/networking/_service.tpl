{{/*
Full Service resource template.
...
metadata:
  ...
  labels:
    {{- include "common.labels" (dict "context" . "labels" .Values.service.labels) | nindent 4 }}
  {{- if or .Values.service.annotations .Values.commonAnnotations }}
  annotations:
    {{- include "common.annotations" (dict "context" . "annotations" .Values.service.annotations) | nindent 4 }}
  {{- end }}
spec:
  type: {{ $serviceType }}
  {{- if or $isHeadless (and (hasKey .Values.service "clusterIP") (ne (.Values.service.clusterIP | default "") "")) }}
...
{{/*
Extra Services template (for generic chart advanced use cases).
Usage: {{- include "common.extraServices" . }}
*/}}
{{- define "common.extraServices" -}}
...
metadata:
  ...
  namespace: {{ include "common.namespace" $ }}
  labels:
    {{- include "common.labels" (dict "context" $ "labels" $service.labels) | nindent 4 }}
  {{- if or $service.annotations $.Values.commonAnnotations }}
  annotations:
    {{- include "common.annotations" (dict "context" $ "annotations" $service.annotations) | nindent 4 }}
  {{- end }}
spec:
  type: {{ $service.type | default "ClusterIP" }}
  {{- $isHeadlessExtra := and (eq ($service.type | default "ClusterIP") "ClusterIP") (eq ($service.clusterIP | default "") "None") }}
  {{- if or $isHeadlessExtra (and (hasKey $service "clusterIP") (ne ($service.clusterIP | default "") "")) }}
